...
function UserForm() {
  ...
  const handleUser = async() => {
    ...
    if(valid){
      try{
        if(userToEdit){
          const res = await axios.put(`https://api.freelancing-projects.com/api/admin/${userToEdit._id}/edit-user`,
          {
          ...
          const res = await axios.post('https://api.freelancing-projects.com/api/admin/create-user',
          {
            name,email,mobile,price:Number(price),expiry:date,admin:admins,packages,paymentoptions:paymentOptions
          },
          {
