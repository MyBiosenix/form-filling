...
const summaryRows = useMemo(() => {
  return comparisonRows
    .filter((r) => {
      const isDouble = doubleEntryRowIds?.has(Number(r.excelRowId));

      // ✅ show normal mistake forms + double entry forms
      return r.mistakes > 0 || isDouble;
    })
    .map((r) => {
      const isDouble = doubleEntryRowIds?.has(Number(r.excelRowId));

      // ✅ double entry counts as 1 extra mistake
      const finalMistakes = Number(r.mistakes || 0) + (isDouble ? 1 : 0);
      ...
      return {
        ...
        actualMistakes: r.mistakes, // optional: original comparison mistakes
        isDoubleEntry: isDouble,
        mistakePercent: `${finalMistakes}%`,
        createdAt: r.createdAt,
      };
    });
}, [comparisonRows, doubleEntryRowIds]);
  ...
  const saveEdit = async () => {
    try {
      ...
      const res = await axios.put(
        `https://api.freelancing-projects.com/api/admin/form-entry/${editEntry._id}`,
        { responses: editDraft },
        { headers: { Authorization: `Bearer ${token}` } }
      );
